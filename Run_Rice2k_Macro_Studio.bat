...
title Rice2k Macro Studio v1.2 Launcher
cd /d "%~dp0"
...
>>"%LOG%" echo ================================================================================
>>"%LOG%" echo [%DATE% %TIME%] Rice2k Macro Studio v1.2 launcher starting
>>"%LOG%" echo Package folder: %CD%

echo.
echo Rice2k Macro Studio v1.2
...
if not exist "rice2k_macro_studio.py" (
    ...
    >>"%LOG%" echo ERROR: rice2k_macro_studio.py is missing.
    pause
    exit /b 9
)
if not exist "src_fragments\part_01.pyfrag" (
    echo ERROR: src_fragments is missing or incomplete.
    >>"%LOG%" echo ERROR: src_fragments is missing or incomplete.
    pause
    exit /b 9
)
...
if not defined PY_CMD (
    ...
    >>"%LOG%" echo ERROR: A working Python installation was not found.
    pause
    exit /b 10
)
